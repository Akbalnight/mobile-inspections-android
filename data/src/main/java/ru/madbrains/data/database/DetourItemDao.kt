...
interface DetourItemDao {
    ...
    @Query("SELECT r.*, COUNT(d.id) as defectCount FROM DetourItemDB as r LEFT JOIN DefectItemDB as d ON d.detourId = r.id AND d.statusProcessId!=:eliminatedId AND d.created = 1 GROUP BY r.id ORDER BY r.dateStartPlan DESC")
